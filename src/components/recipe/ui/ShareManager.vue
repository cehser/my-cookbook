<template>
  <Transition name="sidebar">
    <div v-if="show" class="share-sidebar-backdrop" @click.self="$emit('close')">
        ...
        <div class="sidebar-body">
          ...
          <div class="card mb-3">
            <div class="card-body p-3">
              <label class="form-label small fw-semibold">Neuen Link erstellen</label>
              <div class="d-flex align-items-end gap-2">
                <div class="flex-grow-1">
                  <label class="form-label small text-muted mb-1">Gültig für</label>
                  <select v-model.number="expiresInDays" class="form-select form-select-sm">
          ...
          <!-- Keine Links -->
          <p v-else-if="!shares.length" class="text-muted small text-center py-3">
              ...
              <div class="card-body p-2">
                <div class="d-flex align-items-start gap-2">
                  <div class="flex-grow-1 overflow-hidden">
                    <code class="small user-select-all text-break">{{ shareUrl(share.token) }}</code>
                    <div class="text-muted mt-1" style="font-size: 0.72rem;">
                      Erstellt: {{ formatDate(share.created_at) }}
                    </div>
                    <div v-if="share.expires_at" class="mt-1" style="font-size: 0.72rem;"
                      :class="isExpiringSoon(share.expires_at) ? 'text-warning' : 'text-muted'">
                      <i class="bi bi-clock"></i> Läuft ab: {{ formatDate(share.expires_at) }}
                    </div>
                  </div>
