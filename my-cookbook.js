...
function initRecipe(recipe) {
  ...
  recipe.recipe_uuid  = recipe.recipe_uuid || generateUUID();
...
var app = new Vue({ 
  ...
  methods: {
    ...
    loadYamlRecipe: function (content) {
      var recipe = initRecipe(jsyaml.load(content));
      this.appendRecipe(recipe);
    },
    loadYamlCookbook: function(content) {
      var recipes = jsyaml.load(content);
      recipes.forEach( recipe => {recipe = initRecipe(recipe)});
      return recipes;
    },
    ...
    syncWithWebDAV: async function() {
      ...
      this.recipes = mergeCoobooks(this.recipes, recipes_remote);
