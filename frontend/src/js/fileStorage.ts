 ...
 */
export async function serializeRecipePictures(pictures: RecipePictures): Promise<SerializedRecipePictures> {
 ...
 */
export function deserializeRecipePictures(serialized: SerializedRecipePictures): RecipePictures {
  ...
  for (const [uuid, serializedFiles] of Object.entries(serialized)) {
    if (serializedFiles && serializedFiles.length > 0) {
      pictures[uuid] = serializedFiles.map(sf => 
