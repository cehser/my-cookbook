...
export const aiApi = {
  ...
  async importText(text: string, model: string = 'gpt-4o'): Promise<AIImportResponse> {
  ...
  async importImage(file: File | Blob, filename?: string, model: string = 'gpt-4o'): Promise<AIImportResponse> {
    const headers = await authHeaders()
