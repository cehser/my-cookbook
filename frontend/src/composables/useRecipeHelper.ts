 ...
 */
export function useRecipeHelper(
  options: RecipeHelperOptions,
): RecipeHelperReturn {
  ...
  const yields_unit = computed(() => {
    if (
      current_recipe.value?.yields &&
      current_recipe.value.yields.length > 0
    ) {
  ...
  function recipeImageSrc(recipe: Recipe, size: "thumbnail" | "full"): string {
    const urlFn =
      size === "thumbnail" ? imageApi.thumbnailUrl : imageApi.imageUrl;
  ...
  async function loadRecipe(recipe: Recipe): Promise<void> {
    ...
    if (
      ...
      if (navigator.onLine && (await isAuthenticated())) {
        try {
          const detail = await store.loadRecipeDetailFromApi(
            recipe.recipe_uuid,
          );
    ...
    current_recipe.value.ingredients.forEach((ingredient) => {
      if (
        ...
        ingredient.amounts[0].amount =
          (ingredient.amounts[0].amount * Math.pow(newYield, exp)) /
          Math.pow(oldYield, exp);
      }
    });
  }
